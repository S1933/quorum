...
export function renderMarkdownReport(result: PipelineResult): string {
  const out: string[] = [];
  const allFindings = collectPipelineFindings(
    result.consensus.groups,
    result.consensus.unique,
  );
  ...
  if (result.verdictSummary) {
    out.push(
      `**Plan verdict:** ${escapeMd(result.verdictSummary.decision.toUpperCase())}`,
    );
    out.push('');
  ...
  for (const severity of SEVERITY_ORDER) {
    out.push(
      `| ${severityIcon(severity)} ${severityLabel(severity)} | ${severityCounts[severity]} |`,
    );
  }
  out.push('');
  ...
  if (result.errors.length > 0) {
    ...
    for (const e of result.errors)
      out.push(`- **${escapeMd(e.reviewerId)}**: ${escapeMd(e.message)}`);
    out.push('');
  }
  ...
  if (result.verdictSummary?.summaries.length) {
    ...
    for (const verdict of result.verdictSummary.summaries) {
      const confidence = verdict.confidence
        ...
        : '';
      out.push(
        `- **${escapeMd(verdict.reviewerId)}**: ${escapeMd(verdict.decision)}${escapeMd(confidence)} — ${escapeMd(verdict.summary)}`,
      );
    }
    out.push('');
  }
  ...
  if (totalInput > 0 || totalOutput > 0) {
    out.push(
      `| Reviewer | Input Tokens | Output Tokens | ${hasCost ? 'Cost' : ''}`,
    );
    out.push(`| --- | ---: | ---: |${hasCost ? ' ---:' : ''}`);
    for (const review of result.reviews) {
      if (!review.usage) {
        out.push(
          `| ${escapeMd(review.reviewerId)} | _no data_ | _no data_ |${hasCost ? ' _no data_' : ''}`,
        );
        continue;
      }
      const cost =
        review.usage.costUsd !== undefined
          ? `$${review.usage.costUsd.toFixed(4)}`
          : '—';
      out.push(
        `| ${escapeMd(review.reviewerId)} | ${review.usage.inputTokens.toLocaleString()} | ${review.usage.outputTokens.toLocaleString()} |${hasCost ? ` ${cost}` : ''}`,
      );
    }
    out.push(
      `| **Total** | **${totalInput.toLocaleString()}** | **${totalOutput.toLocaleString()}** |${hasCost ? ` **$${totalCost.toFixed(4)}**` : ''}`,
    );
    out.push('');
  } else {
  ...
  out.push('---');
  out.push(
    `*Generated by quorum · consensus strategy: ${escapeMd(result.consensus.strategyId)}*`,
  );
  return out.join('\n');
}
...
function renderPriorityFindings(
  ...
  for (const bucket of buildSeverityBuckets(groups, unique)) {
    ...
    out.push(
      `### ${severityIcon(bucket.severity)} ${severityLabel(bucket.severity)} (${bucket.count})`,
    );
...
function renderGroup(out: string[], g: FindingGroup): void {
  ...
  out.push('');
  out.push(
    `> ${g.reviewers.length} reviewer(s) agreed · ${g.members.length} matching report(s)`,
  );
  out.push('');
  renderFindingMeta(
    out,
    f,
    `**Reviewers:** ${g.reviewers.map(escapeMd).join(', ')}`,
  );
  out.push('');
...
function renderFindingMeta(
  out: string[],
  f: Finding,
  reviewerLine: string,
): void {
  out.push(
    `- **Location:** \`${escapeFilePath(f.file)}:${f.lineRange.start}-${f.lineRange.end}\``,
  );
