...
| F002 | Security / Performance | `src/providers/opencode-go/index.ts:33` | High | M | Full review prompts are passed as CLI args for opencode; cursor and kilo do the same at `src/providers/cursor-agent/index.ts:35` and `src/providers/kilo-code/index.ts:41`. This leaks diffs via process listings and can hit OS argv limits on large diffs. | Prefer stdin for all subprocess providers; keep only small flags in argv. |
| F003 | Security | `src/providers/claude-code/schema.ts:8` | High | S | `claude-code.extra_args` accepts arbitrary strings. `opencode-go.extra_args` does too at `src/providers/opencode-go/schema.ts:11`, while other providers use allowlists. | Apply provider-specific safe arg allowlists everywhere; forbid flags that alter sandboxing, cwd, config, or execution permissions. |
