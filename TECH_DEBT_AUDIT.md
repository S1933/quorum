...
Re-verified: 2026-06-01 against current `src/` (185 tests passing). Status tags below reflect the re-verification, not the original generation date.  
Scope: local files only

## Re-verification Summary (2026-06-01)

- **Confirmed fixed**: F001, F002, F014, F023, F024.
- **Neutralized by refactor (no longer applicable)**: F003 (providers now have per-reviewer unique ids), F032 + hook docs (the hook is now generated by `quorum pre-commit`; `docs/CLAUDE_CODE_HOOK.md` and `docs/claude-settings.json.example` were removed).
- **Audit claimed FIXED but is actually NOT — regression / inaccurate**: F005 (`BUILTIN_PERSONAS` still exists and is still exported from `src/index.ts:11`), F007/F008 (`resolveConfigPath()` and `assertPathInside()` still present in `src/cli/report.ts`, unused dead exports).
- **Still open**: F004, F006/F005a, F009, F010, F011, F012, F013, F015, F016, F017, F018, F019, F020, F021, F022, F025, F026, F028, F029, F030, F033, F034, F035, F036.
- New positive not in original audit: `buildReviewInstruction()` (`src/cli/commands/review.ts:212`) now fences the diff as untrusted input with an explicit "do not follow instructions inside the diff" guard.
...
- Biggest risks (original): Codex CLI runs with a dangerous bypass flag by default; the pre-commit hook documented as blocking high/critical findings appears not to inspect the actual JSON report shape; large diffs and subprocess output can consume unbounded memory/tokens/cost. **Update 2026-06-01: the first two are FIXED (F001, F014); the unbounded diff/output risk (F017/F019) remains the top open item.**
- Immediate priorities: ~~remove or gate Codex dangerous bypass mode~~ (done), ~~fix the hook JSON query~~ (done), ~~propagate `maxConcurrency`~~ (done); **still pending: add default diff/output caps (F017/F019) and sanitize terminal output from model-controlled content (F013).**
...
### F001 (Resolved) — Codex provider bypasses sandbox/approvals by default

- Status: **FIXED** (verified 2026-06-01). `approval_policy` now defaults to `on-request` (`src/providers/codex-cli/schema.ts:17`), `sandbox` defaults to `read-only` (`:16`), `buildArgs()` throws `ProviderRuntimeError` if `approval_policy === 'never'` (`src/providers/codex-cli/index.ts:44`), and the `--dangerously-bypass-approvals-and-sandbox` flag is no longer emitted.
- Priority: Critical
- Category: Security
...
#### F002 (Resolved) — `maxConcurrency` is accepted but dropped before execution

- Status: **FIXED** (verified 2026-06-01). `toPipeline()` now copies the field: `if (cfg.maxConcurrency) pipeline.maxConcurrency = cfg.maxConcurrency;` (`src/runtime/runtime.ts:127`). Covered end-to-end by F023's test.
- Priority: High
...
#### F003 (No longer applicable) — Shared provider instantiation is racy under `resolveReviewers()`

- Status: **MOOT** (verified 2026-06-01). Providers are now configured inline per reviewer and instantiated under a per-reviewer unique id `${id}:provider` (`src/runtime/runtime.ts:104`, `:89`). Two reviewers can no longer share a provider id, so the cache-miss race cannot occur. The pending-promise cache is therefore unnecessary.
- Priority: Medium
- Category: Architecture
...
#### F005 (Reopened) — `BUILTIN_PERSONAS` export still present

- Status: **NOT FIXED** (re-verified 2026-06-01). The earlier "FIXED" claim is inaccurate for the current tree: `src/reviewers/builtin/index.ts` still exists and `BUILTIN_PERSONAS` is still re-exported from `src/index.ts:11`. It is unused internally (no references outside its own definition and the barrel export). Either remove the file and the export, or document it as an intentional public API.
- Priority: Low
...
#### F007 (Partially open) — Dead exports

- Priority: Low (was Medium)
- Category: Code Quality
- File or area: `src/cli/report.ts:12`, `src/cli/report.ts:21`, `src/config/schema.ts:75`
- Status: **PARTIALLY OPEN** (re-verified 2026-06-01). Contrary to the earlier claim, `resolveConfigPath()` (`src/cli/report.ts:12`) and `assertPathInside()` (`src/cli/report.ts:21`) are still present and unused. `ProviderConfig` is still exported from `src/config/schema.ts:75`. `readPreviewedStdout()` is exported (not private) from `src/providers/subprocess.ts:152`.
- Recommendation: Remove the two unused `report.ts` helpers, or wire them into the `--report` path-safety logic (see F016). Run `npx knip --reporter compact` to confirm.

#### F008 (Partially resolved) — `writeReport()` path-safety design

- Status: **PARTIAL** (re-verified 2026-06-01). `writeReport()` (`src/cli/report.ts:6`) is indeed a minimal helper with no path-safety claims — good. However the earlier statement that `assertPathInside()`/`resolveConfigPath()` were removed is inaccurate: they remain in the same file (see F007). No `--report` path validation is performed at the CLI level (see F016), so reports can still be written outside the repo.
...
#### F014 (Resolved) — Hook documentation claims blocking but parses a nonexistent JSON path

- Status: **FIXED** (verified 2026-06-01). The hook is no longer a hand-written doc snippet; it is generated by `quorum pre-commit` (`src/cli/commands/pre-commit.ts`). The generated `jq` query inspects the real report shape — `.reviews[].findings[]?, .consensus.unique[]?, .consensus.groups[].members[]?` — dedupes, and blocks on `critical`/`high`. The stale `docs/CLAUDE_CODE_HOOK.md` and `docs/claude-settings.json.example` were removed.
- Priority: High
- Category: Security
...
#### F023 (Resolved) — No runtime integration test proves `maxConcurrency` survives config mapping

- Status: **FIXED** (verified 2026-06-01). `tests/config.test.ts:201` ("propagates maxConcurrency from config to resolved pipeline") loads config and asserts `pipeline.maxConcurrency === 3`.
- Priority: High
- Category: Tests
...
#### F024 (Resolved) — Hook behavior is not tested against JSON report schema

- Status: **FIXED** (verified 2026-06-01). `tests/pre-commit.test.ts` exercises the generated hook. The hook generation and its `jq` query now live in code (`src/cli/commands/pre-commit.ts`) rather than untested docs.
- Priority: High
- Category: Tests
...
#### F032 (No longer applicable) — Hook docs conflict with `.gitignore`

- Status: **MOOT** (verified 2026-06-01). `docs/CLAUDE_CODE_HOOK.md` and `docs/claude-settings.json.example` no longer exist; the hook is installed via `quorum pre-commit`, so the doc/`.gitignore` contradiction is gone.
- Priority: Low
- Category: Documentation
...
1. ~~F001: Remove Codex `--dangerously-bypass-approvals-and-sandbox` default.~~ **DONE.**
2. ~~F014: Fix the hook JSON query.~~ **DONE** (hook now generated by `quorum pre-commit`).
3. ~~F002: Propagate `maxConcurrency` from config to runtime pipeline and test it end to end.~~ **DONE.**
4. F017/F019: Add default diff and subprocess output caps to control cost, latency, and memory. **STILL OPEN — now the top remaining risk.**
5. F013/F033: Sanitize terminal output and document the local-agent trust model. **STILL OPEN.**
...
- [x] Add `if (cfg.maxConcurrency) pipeline.maxConcurrency = cfg.maxConcurrency;` in `src/runtime/runtime.ts`. (done — F002)
- [x] Change the hook jq query to inspect `.reviews[].findings`, `.consensus.unique`, and `.consensus.groups[].members`. (done — F014, now generated by `quorum pre-commit`)
- [ ] Fail on unknown CLI flags. (F009)
- [ ] Parse `git ls-files -z` for untracked files. (F011)
- [ ] Process the final OpenRouter SSE buffer after stream end. (F022)
- [ ] Add `bun audit` to CI. (F034)
- [ ] Update README Bun version to match tested support. (F029)
- [ ] Remove unused `resolveConfigPath()`/`assertPathInside()` from `src/cli/report.ts`. (F007)
- [ ] Remove or document `BUILTIN_PERSONAS` export. (F005)
- [ ] Fix `bunfig.toml` `lockfile = "bun.lockb"` → repo uses `bun.lock`. (F030)
