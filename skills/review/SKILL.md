---
name: review
description: Run the configured Quorum review pipeline against the current diff and render the exact consensus report. Use when the user asks for a Quorum review, consensus review, or multi-reviewer review of local code changes.
---
...
2. Keep the command working directory on that target root; do not `cd` into the Quorum repo.
3. Parse raw arguments into an argv list before running Bash:
...
5. Invoke with each argv token shell-quoted individually:
   ...
   - Omit quoted argument placeholders when there are no parsed args.
6. Run the CLI in a single Bash command so stdout/stderr are captured together.
7. Do not summarize, classify, rewrite, or paraphrase findings.
8. Final response format:
   - First, a fenced `text` block containing exact CLI stdout/stderr.
   - Then, if `.quorum/last-review.md` exists, a `## Report` heading followed by exact report markdown.
   - No extra success banner, no `Summary`, no rewritten `Critical issues` section.

## Failure Modes

- Missing `quorum.yaml`: tell the user where to put it, and point at `quorum.yaml.example`.
- Missing env vars, such as `OPENROUTER_API_KEY`: cite the variable name and provider that needs it.
...
This skill is a thin shell. All domain logic lives in `src/`.
